...
const ATTACHMENT_COLLECTIONS = new Set(["notes", "files", "assignments", "labReports"]);

/**
 * Strip heavy fileData blobs from list responses.
 * Returns lightweight metadata-only records unless fields=full is requested.
 */
function stripFileData(collection, data) {
  if (!ATTACHMENT_COLLECTIONS.has(collection) || !Array.isArray(data)) return data;
  return data.map((item) => {
    if (!item || typeof item !== "object" || !item.fileData) return item;
...
export async function onRequestGet(context) {
  ...
  // Single-item fetch: ?id=123 — returns full record including fileData.
  ...
  // List fetch: strip fileData by default unless ?fields=full is requested.
  const fields = url.searchParams.get("fields");
  const responseData = fields === "full" ? data : stripFileData(collection, data);
