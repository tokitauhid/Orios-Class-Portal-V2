...
// ── Session cache ──
// Avoid re-fetching entire collections on every page navigation.
const CACHE_TTL_MS = 2 * 60 * 1000; // 2 minutes

function cacheKey(collection) {
  return `orios_cache_${collection}`;
}

function getCached(collection) {
  try {
    const raw = sessionStorage.getItem(cacheKey(collection));
    if (!raw) return null;
    const { data, ts } = JSON.parse(raw);
    if (Date.now() - ts > CACHE_TTL_MS) {
      sessionStorage.removeItem(cacheKey(collection));
      return null;
    }
    return data;
  } catch {
...
function setCache(collection, data) {
  try {
    sessionStorage.setItem(cacheKey(collection), JSON.stringify({ data, ts: Date.now() }));
...
/**
 * Fetch a single record by ID directly from the API.
 * Unlike getOne(), this hits the ?id= endpoint and returns the full
 * record including fileData (not stripped).
 */
export async function getOneById(collectionName, id) {
  if (await isApiAvailable()) {
    try {
      const res = await fetch(
        getAPIUrl(`${API_BASE}?collection=${collectionName}&id=${encodeURIComponent(id)}`),
        { cache: "no-store", headers: authHeaders() }
      );
      if (res.ok) return await res.json();
    } catch { /* Fall through. */ }
  }
  // Fallback: search cached list (won't have fileData, but better than nothing).
