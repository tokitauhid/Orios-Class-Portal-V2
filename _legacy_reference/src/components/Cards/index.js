...
import { getOneById } from "@site/src/auth";
...
export function FileShareCard({ file, delay = 0 }) {
  ...
  const triggerDownload = async () => {
    ...
    } else if (file._hasFile) {
      setDownloading(true);
      try {
        const full = await getOneById("files", file.id);
        if (full?.fileData) {
          const a = document.createElement("a");
          ...
          showToast("File data could not be loaded.", "info");
        }
      } catch (err) {
        console.error("Failed to fetch file:", err);
        showToast("Failed to download file.", "info");
      } finally {
    ...
    <>
      <div className={styles.fileCard} style={{ animationDelay: `${delay}ms` }}>
        ...
        <div className={styles.fileFooter}>
          ...
          <button className={styles.fileDownloadBtn} onClick={handleDownload} disabled={downloading}>
            {downloading ? "⏳ Loading..." : file.password && !unlocked ? "🔒 Download" : "⬇ Download"}
          </button>
        </div>
      </div>
