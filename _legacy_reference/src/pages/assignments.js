...
import { getAll, autoUpdateStatuses, getOneById } from "@site/src/auth";
...
export default function AssignmentsPage() {
  ...
  const handleDownload = useCallback(async (item) => {
    if (item.fileData) {
      ...
      a.download = item.title + "." + (item.format || "bin").toLowerCase();
    ...
    if (item._hasFile) {
      ...
      try {
        const full = await getOneById("assignments", item.id);
        if (full?.fileData) {
          const a = document.createElement("a");
  ...
  const filtered = (() => {
    ...
    if (subjectFilter !== "all") {
      base = base.filter((a) => a.subject === subjectFilter);
    }
    return base;
  })();
                      ...
                      >
                        ...
                        <div className={styles.cardFooter}>
                          <div className={styles.footerMeta}>
                            ...
                            {(a.fileData || a._hasFile) && (
                              <button
                                className={styles.downloadBtn}
                                onClick={() => handleDownload(a)}
                                disabled={loadingFileId === a.id}
                              >
                                {loadingFileId === a.id ? "⏳ Downloading..." : "⬇ Download Attachment"}
                              </button>
                            )}
                          </div>
                        </div>
